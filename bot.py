...
FAQ_TEXT = """
❓ **ЧАСТЫЕ ВОПРОСЫ:**
1️⃣ **Сколько вузов?** 5 вузов, до 5 направлений в каждом.
2️⃣ **Что такое Приоритет?** Рейтинг желаний. Зачислят на высший по списку, куда проходишь.
3️⃣ **Оригинал аттестата?** Нужен до 3 августа (12:00 МСК) в тот вуз, куда проходишь.
4️⃣ **Вторая волна?** Нет, зачисление в одну волну!
"""

# Справочник профессий по предметам
SUBJECTS_INFO = {
...
def show_professions(message):
    text = SUBJECTS_INFO[message.text]
...
def send_pamphlet(message):
    ...
    else: bot.send_message(message.chat.id, "Загрузка...")

@bot.message_handler(func=lambda m: m.text == "⏳ Таймер")
def timer(message):
    days = (datetime(2025, 5, 23) - datetime.now()).days
    bot.send_message(message.chat.id, f"🔥 До ЕГЭ осталось **{days} дней**!" if days > 0 else "Экзамены начались!", parse_mode="Markdown")
