...
# --- ПРОЕКТ РАСПИСАНИЯ ЕГЭ 2026 ---
# Даты ориентировочные (основной период)
EXAM_DATES = {
    "География/Лит/Хим": "2026-05-22",
    "Русский язык": "2026-05-26",
    "Математика (Б/П)": "2026-05-29",
    "Общество/Физика": "2026-06-02",
    "История": "2026-06-05",
    "Биология/Ин.яз": "2026-06-09",
    "Информатика (КЕГЭ)": "2026-06-16"
}
...
# --- ОБРАБОТКА ТАЙМЕРА (ОБНОВЛЕНО ДЛЯ 2026) ---
@bot.message_handler(func=lambda m: m.text == "⏳ Таймер до ЕГЭ")
def timer_menu(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row("Русский язык", "Математика (Б/П)")
    markup.row("Общество/Физика", "Информатика (КЕГЭ)")
    markup.row("История", "Биология/Ин.яз")
    markup.row("География/Лит/Хим", "🔙 В меню")
    
    bot.send_message(message.chat.id, "⏰ До какого экзамена 2026 года считаем дни?", reply_markup=markup)
...
def show_timer(message):
    ...
    if delta.days > 0:
        bot.send_message(message.chat.id, 
                         ...
                         f"Используй это время с умом!", 
                         parse_mode="Markdown")
    else:
        bot.send_message(message.chat.id, f"Экзамен **{subject}** уже прошел или идет сегодня!", parse_mode="Markdown")
